...
- 已新增多路线预览接口 `/api/optimize/multi-preview`；
- 已新增独立的“多路线生成”页面，输入从单条路线调整为公司点位池；
- 已支持按额定载重、目标装载率和最大趟数拆分路线；
- 已使用点位预计垃圾量作为容量截停依据；
- 公司级生成已从 `ljszy_facility_info` 读取停车场、中转站、处置场候选，优先用停车场作为起点、处置场作为终点；
- 公司点位池限定为 `facility_type=6` 的收集点，避免把停车场、处理厂等场站纳入待收运点；
- 未维护场站坐标时仍可手动输入临时起终点坐标，未输入时暂用点位中心作为锚点；
- 尚未接入真实车辆排班表和道路 OD。
